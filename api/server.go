...
func NewServer(store sqlc.Store, tokenSecret string, tokenMaker auth.TokenMaker) *Server {
	...
	project.GET("/:projectid/users", server.GetUsersInProject)
	project.POST("/:projectid/user/:username", server.AddUserToProject)
	project.DELETE("/:projectid/user/:username", server.DeleteUserFromProject)
